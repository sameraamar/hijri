...
export default function TodayPage() {
  ...
  const hijriCurrent = useMemo(() => {
    ...
    const center = new Date(Date.UTC(anchor.year, anchor.month - 1, anchor.day, 0, 0, 0));
  ...
  const exportToIcs = () => {
    ...
    const ics = buildIcal(events, t('today.holidayCalendarName'));
        ...
        <section className="card p-4 sm:p-6">
          <div className="flex items-start justify-between gap-3">
            ...
            <button
              ...
              onClick={exportToIcs}
              className="btn-sm whitespace-nowrap"
              aria-label={t('today.addHolidayToCalendar')}
              title={t('today.addHolidayToCalendar')}
            >
              ...
              <span className="hidden sm:inline">{t('today.addHolidayToCalendar')}</span>
            </button>
          </div>
        </section>
      ) : null}
